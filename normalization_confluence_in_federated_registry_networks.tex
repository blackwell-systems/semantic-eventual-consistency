...
Our key construction reduces the federated problem to the single-registry
case: a federation of registries connected by morphisms \emph{is} a registry,
with morphism consistency conditions as additional invariants and morphism
repair as additional compensation.  For \emph{tree-shaped} morphism networks
(a directed forest: each non-root registry has exactly one incoming morphism),
federated convergence requires only validity preservation of the
morphisms---all other conditions are \emph{derived} from the network's
acyclicity and the components' existing convergence properties.  The core
mechanism is an \emph{authority argument}: in a directed morphism, the
source's unique normal form deterministically determines the target's shared
component, eliminating the nondeterminism that would otherwise require
additional axioms.

We prove both structural conditions necessary by counterexample: without
acyclicity, contradictory cyclic morphisms cause compensation to cycle;
without validity preservation, morphism repair and local compensation
oscillate.  For self-containment, we restate the single-registry model and
convergence results of~\cite{blackwell2026nc} alongside a verification
calculus that reduces the global CC obligation to local, per-event-pair checks
via invariant footprints and decomposable repair.
\end{abstract}
...
registry whose convergence reduces to the single-registry case.  For
tree-shaped morphism networks, this reduction requires surprisingly little
beyond the component conditions: only validity preservation of the morphisms.
...
federated construction, the authority argument, and the federated convergence
theorem with necessity counterexamples.  For self-containment, we restate the
...
restated here for self-containment.  The federated extension (item~7) is new.
...
\item \textbf{Federated convergence} (\Cref{sec:federation}): Extension to
  ...
  Federated WFC and CC are derived from network structure via an authority
  argument; only morphism validity preservation is needed.  Necessity of
  both acyclicity and validity preservation is shown by counterexample.
\end{enumerate}
...
additional compensation.  Federated convergence then follows from
\Cref{thm:convergence} if the federated registry satisfies WFC and CC.  We
prove both are derived from the network's structure rather than assumed.
...
image of a locally valid source preserves target validity.  Federated events
target a specific registry~$R_m$ and modify only component~$m$; cross-registry
effects arise through morphism violation and repair.  The federated governance
rewrite system $\mathcal{G}_\Fed$ has apply steps and compensation steps as in
the single-registry case (\Cref{def:grs}).
\end{definition}

\subsection{The Authority Argument}

The key insight is that in a directed morphism $\phi: R_A \to R_B$,
registry~$A$ is \emph{authoritative} over $B$'s shared component: the
federated normal form of $B$'s shared state is completely determined by
$A$'s normal form.  The federated normal form decomposes into three kinds
of state components, each with a separate convergence argument.

\begin{lemma}[Authority Determination]
\label{lem:authority}
In a federally valid state $\bar{\sigma}$:
\begin{enumerate}[leftmargin=*,nosep,label=(\alph*)]
\item Each source registry's state equals its single-registry normal form
  (events targeting other registries and morphism repair do not modify
  source components).
\item The shared component of non-source~$j$ with incoming edge from~$i$
  satisfies $\pi_j^{\mathrm{sh}}(\sigma_j) = \phi_{ij}(\sigma_i)$
  (determined by source normal forms propagated through morphisms).
\item The local component of non-source~$j$ is unconstrained by morphisms
  and converges via component CC for~$R_j$.
\end{enumerate}
\end{lemma}

\begin{lemma}[Single-Round Termination]
\label{lem:fed-termination}
A single application of $\rho_\Fed$ produces a federally valid state.
Phase~1 achieves local validity in $\leq \sum_i M_i$ steps; Phase~2 processes
each edge once (unique target: each registry's shared component is set once
and not revisited).  Critically, morphism repair cannot reintroduce local
invalidity: M1 guarantees that overwriting the shared component of a locally
valid target with the morphism image of a locally valid source preserves
target validity.  Since Phase~1 establishes local validity before Phase~2
begins, and Phase~2 only modifies shared components, local validity is an
invariant of the entire procedure.
\end{lemma}

\begin{theorem}[Federated CC]
\label{thm:fed-cc}
Let $\mathcal{N}$ be a tree-shaped morphism network where each component
satisfies WFC and CC.  Then the federated system satisfies CC.
\end{theorem}

\begin{proof}
We verify CC1 and CC2 by decomposing the federated normal form into three
component types per \Cref{lem:authority}.

\textbf{CC2 (Compensation absorption):} Let $\bar{\sigma}$ be federally
invalid and $e$ an event targeting~$R_m$.  Source normal forms are determined
by component CC2 (if $e$ targets a source) or unaffected (otherwise).  Shared
components of non-sources are deterministic functions of source normal forms,
which agree between the two CC2 paths.  Local components of non-sources
converge by component CC2 (morphism repair does not modify local components).

\textbf{CC1 (Order independence):} Let $e_1, e_2$ be independent.  If both
target the same source, component CC1 gives agreement; if they target
different registries, each source's normal form depends only on its own
events.  Shared components follow from source agreement via
\Cref{lem:authority}(b).  Local components: if both target the same
non-source, component CC1 gives agreement; otherwise each local component
sees at most one event. \qed
\end{proof}

\begin{theorem}[Federated Normalization Confluence]
\label{thm:fed-convergence}
Let $\mathcal{N}$ be a tree-shaped morphism network where each morphism
satisfies M1 and each component registry satisfies WFC and CC.  Then any two
processors consuming the same federated events converge to the same federally
valid state.
\end{theorem}

\begin{proof}
By \Cref{lem:fed-termination}, $\rho_\Fed$ is a one-shot normalizer: it maps
any federated state to a federally valid state in a single application (since
morphism repair modifies only shared components and M1 preserves local
validity, Phase~2 cannot increase the compensation measure of any component).
The induced rewrite system satisfies WFC with trivial federated measure
$\Phi_\Fed(\bar{\sigma}) = \mathbf{1}[V_\Fed(\bar{\sigma}) = \bot]
\in \{0, 1\}$; a single compensation step always reaches validity.
Termination of the full rewrite system (apply + compensate) follows from
the lexicographic order $(|B|, \Phi_\Fed(\bar{\sigma}))$.
By \Cref{thm:fed-cc}, the system satisfies CC.
\Cref{thm:convergence} gives unique normal forms.
\qed
\end{proof}

\begin{corollary}[Constructive Normal Form]
\label{cor:fed-nf}
The federated normal form is constructive: compute each source's
single-registry normal form independently, propagate shared components through
morphisms in topological order, and let each non-source converge its local
component via component CC.
\end{corollary}
...
registries with no morphism constraints.  Federated convergence generalizes
product composition to the case where registries are connected by directed
...
or conflict resolution operators) and is the natural next problem.
\end{remark}
...
Moreover, this rewrite-system perspective extends naturally across
organizational boundaries: the federated convergence result
(\Cref{sec:federation}) shows that multi-registry systems inherit convergence
from their components, with morphism repair as additional rewrite rules in the
same framework.
\end{remark}
...
morphisms encode specification conformance.  \Cref{thm:fed-convergence}
guarantees that all participants converge to consistent states across
...
uniform complexity bounds.  All three conditions are necessary for their
respective guarantees: without UBC, compensation depth is unbounded; without
CC, different orders reach different valid states.

The result extends across organizational boundaries.  For tree-shaped networks
of registries connected by morphisms, federated convergence requires only
validity preservation of the morphisms: federated termination and commutativity
are derived from the network's acyclicity and the components' existing
convergence properties, not assumed as additional axioms.  The core mechanism
is the authority argument: in a directed morphism, the source's unique normal
form deterministically determines the target's shared component, eliminating
the nondeterminism that would otherwise require additional conditions.
